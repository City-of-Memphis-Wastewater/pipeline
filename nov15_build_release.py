...
def main():
    ...
    # PEX (skip on Termux)
    if not sysinfo.is_termux():
        pex_path = DIST_DIR / f"{base_name}.pex"
        try:
            build_pex(wheel, pex_path, args.entry_point)
        except Exception as e:
            print(f"PEX failed (Termux?): {e}")

    # --- Launchers ---
    if not sysinfo.is_termux():
        pyz_final = shiv_path if args.shiv else zipapp_path
        make_bat(pyz_final, pyz_final.with_suffix(".bat"))
        make_macos_app(pyz_final, pyz_final.with_suffix(".app"))
