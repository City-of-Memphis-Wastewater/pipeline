...
def ensure_user_db() -> Path:
    ...
    if not user_db.exists() and packaged_db.exists():
        with resources.as_file(resources.files("pipeline_eds.data") / "sensors.db") as packaged_db:
...
#def get_packaged_db_path() -> Path:
#    packaged_db = resources.as_file(resources.files("pipeline_eds.data") / "sensors.db")
#    return packaged_db
