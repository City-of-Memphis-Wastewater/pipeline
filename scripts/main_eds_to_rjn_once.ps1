...
Start-Process -FilePath $venvPython `
    -ArgumentList "-m", "workspaces.eds_to_rjn.scripts.daemon_runner", "once" `
    -WorkingDirectory $workingDir `
    -WindowStyle Hidden `
    -RedirectStandardOutput (Join-Path $logDir "daemon_eds_to_rjn_output_once.log") `
    -RedirectStandardError  (Join-Path $logDir "daemon_eds_to_rjn_error_once.log") `
	-Environment $envDict
